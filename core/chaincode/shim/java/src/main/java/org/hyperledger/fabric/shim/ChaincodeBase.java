...
package org.hyperledger.mchain.shim;

import static org.hyperledger.mchain.shim.Chaincode.Response.Status.INTERNAL_SERVER_ERROR;
import static org.hyperledger.mchain.shim.Chaincode.Response.Status.SUCCESS;
...
import org.hyperledger.mchain.protos.peer.ChaincodeSupportGrpc;
import org.hyperledger.mchain.protos.peer.ChaincodeSupportGrpc.ChaincodeSupportStub;
import org.hyperledger.mchain.shim.impl.Handler;
import org.hyperledger.mchain.shim.impl.NextStateInfo;
...
public abstract class ChaincodeBase implements Chaincode {
	...
	private String rootCertFile = "/etc/hyperledger/mchain/peer.crt";
	...
	public void chatWithPeer(ManagedChannel connection) {
		...
		try {
			requestObserver = stub.register(new StreamObserver<ChaincodeMessage>() {
				...
				public void onNext(ChaincodeMessage message) {
					...
					try {
						logger.debug(String.format("[%-8s]Received message %s from org.hyperledger.mchain.shim", message.getTxid(), message.getType()));
		...
		// Create the org.hyperledger.mchain.shim handler responsible for all
		// control logic
